...
library UniswapV2Library {
    ...
    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) internal pure returns (uint256 amountB) {
        ...
        //数额B = 数额A * 储备量B / 储备量A
        // quote 函数的计算方法实际上是根据两种资产的比例关系来计算的，不是按照恒定乘积公式计算的，即 amountA / amountB = reserveA / reserveB。
...
contract UniswapV2Router01 is IUniswapV2Router01 {
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            // 下面的思路是：先按照某一个期望的token为锚点（比如 amountADesired），计算另一种 token 可以获得的数量
            //最优数量B = 期望数量A * 储备B / 储备A
     ...
     * @param amountAMin 最小数量A
     * @param amountBMin 最小数量B
     * @param to to地址（即配对合约中 mint 函数中的to地址）
     * @param deadline 添加流动性操作的最后期限
     * @return amountA   数量A
     * @return amountB   数量B
    ...
    function swapTokensForExactTokens(
        ...
        //确认数额数组第一个元素<=最大输入数额
        require(amounts[0] <= amountInMax, "UniswapV2Router: EXCESSIVE_INPUT_ AMOUNT");
        //将数量为数额数组[0]的路径[0]的token从调用者账户发送到路径0,1的pair合约
        TransferHelper.safeTransferFrom(
